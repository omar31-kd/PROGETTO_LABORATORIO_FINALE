                          ...
                          Tecnico **tecnici, int nt)
{
    ...
    for (i = 0; i < nr; i++) {
        if (richieste[i] != NULL) {
            cnt_stato[(int)richiesta_get_stato(richieste[i])]++;
            cnt_prior[(int)richiesta_get_priorita(richieste[i])]++;
    ...
    for (i = 0; i < nr; i++) {
        ...
        fprintf(fp,
            ...
            richiesta_get_codice(r), richiesta_get_cliente(r),
            richiesta_get_dispositivo(r),
            PCLS[(int)richiesta_get_priorita(r)], PLBL[(int)richiesta_get_priorita(r)],
            SCLS[(int)richiesta_get_stato(r)],    SLBL[(int)richiesta_get_stato(r)],
            richiesta_get_costo_stimato(r));
